...
class Serializer {
    ...
    static typeCanAssign(type) {
        return type !== NetworkType.SERIALIZABLE_OBJECT && type !== NetworkType.LIST;
    }
    ...
    writeDataView(dataView, value, bufferOffset, netSchemProp) {
        if (netSchemProp.type === NetworkType.FLOAT32) {
        ...
        } else if (netSchemProp.type === NetworkType.SERIALIZABLE_OBJECT) {
            ...
            for (let item of value) {
                ...
                if (netSchemProp.itemType === NetworkType.SERIALIZABLE_OBJECT) {
